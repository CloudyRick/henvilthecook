            ...
            {(sections as ContentSection[] | null)?.map((section) => (
              <div key={section.id} className="rounded-lg border border-gray-200 p-4">
                ...
                <div className="mt-4 space-y-3 rounded-lg border border-gray-100 bg-gray-50 p-4">
                  ...
                  {/* Existing files */}
                  {(filesBySection[section.id] ?? []).length > 0 && (
                    <div className="space-y-2">
                      {(filesBySection[section.id] ?? []).map((f) => (
                        <div key={f.id} className="flex items-center justify-between rounded-lg border border-gray-200 bg-white px-3 py-2">
                          <div className="flex items-center gap-2">
                            <span className="rounded px-1.5 py-0.5 text-xs font-medium" style={{ background: f.type === "image" ? "#fef3c7" : "#dbeafe", color: f.type === "image" ? "#92400e" : "#1e40af" }}>
                              {f.type}
                            </span>
                            <span className="text-sm text-gray-700">{f.name}</span>
                          </div>
                          <form action={deleteSectionFile}>
                  ...
                  <SectionFileUpload sectionId={section.id} />
                </div>
